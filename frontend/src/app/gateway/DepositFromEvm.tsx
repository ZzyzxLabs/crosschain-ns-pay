...
export function DepositFromEvm() {
  const [state, action, pending] = useActionState(depositFromEvmAction, initialState);

  return (
    <section className="rounded-2xl border border-white/10 bg-white/5 p-6">
      <h2 className="text-lg font-semibold text-white">Deposit from EVM</h2>
      <p className="mt-1 text-sm text-slate-300">Approve and deposit USDC into the Gateway wallet.</p>
      ...
      {state.data?.txHash ? (
        <p className="mt-2 break-all text-xs text-slate-400">Tx: {state.data.txHash}</p>
      ) : null}
    </section>
  );
}
